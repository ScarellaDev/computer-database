    ...
    if (!StringValidator.isEmpty(computerDto.getIntroduced())) {
      if (!StringValidator.isDate(computerDto.getIntroduced(), "yyyy-MM-dd")) {
        errorMap
            .put(
                "eDateI",
                "Incorrect introduced date: the field must be at the yyyy-MM-dd format (within a range of '1970-01-02' UTC to '2038-01-19' UTC) or shoulb be left empty");
      }
    }
    ...
    if (!StringValidator.isEmpty(computerDto.getDiscontinued())) {
      if (!StringValidator.isDate(computerDto.getDiscontinued(), "yyyy-MM-dd")) {
        errorMap
            .put(
                "eDateD",
                "Incorrect discontinued date: the field must be at the yyyy-MM-dd format (within a range of '1970-01-02' UTC to '2038-01-19' UTC) or should be left empty");
      }
    }
